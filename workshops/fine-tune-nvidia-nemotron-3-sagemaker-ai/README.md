...
Choose **serverless customization of Nemotron 3 Nano 30B-A3B** or **SageMaker Training jobs for Nemotron 3 Nano 4B**. These are alternative SFT tracks, not consecutive labs; the Training jobs track currently has an evaluation integration gap described below.
...
| 3 (pending adaptation) | [3-evaluation.ipynb](02-smtj-workshop/3-evaluation.ipynb) | Retained serverless evaluation workflow; not yet integrated with this track's artifacts or endpoint. Skip in the current execution sequence. |
| 4 (run after 2) | [4-deployment.ipynb](02-smtj-workshop/4-deployment.ipynb) | Discover a completed Training job, deploy its merged artifact directly with vLLM, check a response, and clean up. |

**Evaluation gap:** Notebook 3 still calls `DataSet.get()` for a registered test dataset and looks up a Model Package Group. This track's notebooks 1 and 2 create neither resource: they use plain S3 training channels and Training job artifacts. Notebook 3 is therefore not a working continuation of notebook 2. Deploy with notebook 4 first; full endpoint-based evaluation requires a future adaptation of notebook 3. Notebook 4's serving check is not a replacement for held-out evaluation.
...
- For baseline/judge evaluation, Bedrock invocation access to `global.anthropic.claude-sonnet-4-6` and evaluation access to `amazon.nova-pro-v1:0`, including the applicable model/inference-profile permissions and quotas. These are not required solely to train and deploy Option 2.
...
5. **Option 2:** Run notebooks **1 → 2 → 4**, waiting for the Training job to reach `Completed` before deployment. **Skip notebook 3 as currently written**; deploy with notebook 4 before any future endpoint-based evaluation adaptation.
6. Run the deployment notebook's cleanup cells when finished and verify that the resources were deleted.
